...
void work() {
    ...
    vector<ll> d(n+1);
    rep(i, 1, n+1) cin >> d[i];
    Node storage = Node(d, 1, n+1);
    Node states = Node(1, n+1);
    states.set(1, n+1, 1);
    for ( int i = 0 ; i < n ; i++ ) {
        ll in;
        cin >> in;
        ll grabLocation = states.walk(in);
//        cout << grabLocation << endl;
        cout << storage.sumquery(grabLocation, grabLocation+1);
        states.set(grabLocation, grabLocation+1, 0);
        storage.set(grabLocation, grabLocation+1, 0);
        if ( i < n-1 ) cout << " ";
        else cout << endl;
    }
}
