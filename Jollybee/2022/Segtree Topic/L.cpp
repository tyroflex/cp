...
struct Node {
    ...
    Node(ll start, ll lo, ll hi) : lo(lo), hi(hi) {
        sumval = (hi-lo) * start;
        maxval = start; minval = start;
    }
...
void work() {
    ...
    ll ans = 0;
    Node segtree = Node(0, 1, 10000001);
    while ( !st.empty() ) {
        pair<ll, ll> now = st.top();
        st.pop();
        if ( segtree.sumquery(now.first, now.second+1) < ( now.second - now.first + 1) ) ans++;
        segtree.set(now.first, now.second+1, 1);
    }
    cout << ans << endl;
}
